...
$buildConfig = Get-Content -LiteralPath (Join-Path $projectRoot 'app\build.gradle.kts') -Raw
if ($buildConfig -notmatch 'versionName\s*=\s*"([A-Za-z0-9][A-Za-z0-9.-]*)"') {
    throw '无法从 app/build.gradle.kts 读取版本号'
}
$releaseVersion = $Matches[1]
$releaseDirectory = Join-Path $projectRoot "artifacts\release\$releaseVersion"
$releaseApk = Join-Path $releaseDirectory "Viewer-Android-arm64-v$releaseVersion.apk"
